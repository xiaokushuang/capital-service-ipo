            ...
            :row-class-name="tableRowClassName" :header-cell-class-name="tableHeaderColor" @sort-change="sortChange">
              ...
              <el-table-column label="辅导情况" align="center"  >
                <el-table-column align="center" :render-header="renderHeader" style="height:54px;" prop="fdcx" border="true" label="持续辅导" sortable="custom">
                  <template slot-scope="scope">
                    <span class="spanClass" v-if="getValue(scope.row.fdcx) != 0" @click="openFdDetailProv(scope.row.registAddr ?  scope.row.registAddr:'fd','','0','unit')">{{scope.row.fdcx}}</span>
                ...
                <el-table-column align="center" :render-header="renderHeader" prop="fdwc" label="辅导完成（未受理）" sortable="custom" >
                  <template slot-scope="scope">
                    <span class="spanClass" v-if="getValue(scope.row.fdwc) != 0" @click="openFdDetailProv(scope.row.registAddr ?  scope.row.registAddr:'fd','','1','unit')">{{scope.row.fdwc}}</span>
