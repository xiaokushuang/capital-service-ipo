            ...
            <el-col :span='4'>
					...
					<el-row :gutter="24">
						<el-col :span='8'>
							<el-select ref="selectProcess" v-model="caseStatus" title="IPO进程" placeholder="IPO进程" size="small full" :tselect=true
							 @visible-change="calls()" @sure-click="sure('selectProcess')" @keydown.enter.native="querySearch" @clear-click="clearLocal('treeProcess')">
								<el-option :label="caseStatus" :value="caseStatusValue">
									<el-tree :data="processList" show-checkbox node-key="id" ref="treeProcess" highlight-current :props="default_tree"
									 @check-change="selectHandleNodeClick('caseStatus','treeProcess')"></el-tree>
								</el-option>
							</el-select>
						</el-col>
						<el-col :span='8'>
							<el-select ref="selectVerifyResult" v-model="iecResult" title="审核/注册结果" placeholder="审核/注册结果" size="small full"
							 ...
							 @clear-click="clearLocal('treeVerifyResult')">
								<el-option :label="iecResult" :value="iecResultValue">
									<el-tree :data="verifyResultList" default-expand-all show-checkbox node-key="id" ref="treeVerifyResult"
									 highlight-current :props="default_tree" @check-change="selectHandleNodeClick('iecResult','treeVerifyResult')"></el-tree>
								</el-option>
							</el-select>
						</el-col>
						<el-col :span='7'>
              ...
              <el-multiple-selection class="prospectus" v-if="prospectusShow" :range="true" :tree-data="optionProspectus" placeholder="招股书最近一次估值" size="small full" :multiple="false"
					...
					<el-row :gutter="24">
						...
						<el-col :span='8'>
							<el-multiple-selection v-if="durationShow" :range="true" :tree-data="optionAuditDuration" placeholder="审核历时（天）"
							...
							<div class="table">
								<el-table :cell-class-name="cellStyle" @sort-change="sortChange" :data="tableData" style="width: 100%"
								 v-loading="tableLoading" ref="tables" @row-click="itemClickHandler" class="case" element-loading-text="给我一点时间">
									...
									<el-table-column align="left" prop="ipo_label_result_t" label="进程" sortable="custom" width="100">
										<template slot-scope="scope">
