	...
	export default {
		...
		beforeCreate() {
			if (this.$route.query != "undefined") {
				this.$store.commit('SET_TOKEN', {
					...
					info: this.$route.query['tenant_info'],
		...
		mounted() {
			...
			document.body.addEventListener('mousedown', function(e) {
				t.$store.commit('SET_PARENT_COOKIE', {
		...
		methods: {
			setData(data) {
				console.log(data[3])
				if (data[3] == 'commit') {
					this.$store.commit(data[0] + '/' + data[1], data[2])
				}
				if (data[3] == 'dispatch') {
					this.$store.dispatch(data[0] + '/' + data[1], data[2])
				}
			},
